...
if not exist "%PORTABLE_PYTHON%\python.exe" (
	...
	curl.exe -L -O https://github.com/MARENOL/docs/releases/download/python27-portable/python-2.7.18.amd64.portable.7z
...
if not exist "%~dp0hhc.exe" (
	...
	pause
	curl.exe -L -O https://github.com/MARENOL/deps/releases/download/dependencies/hhc-4.74.8702.7z
...
hhc.exe .\_build\htmlhelp\MARENOLdoc.hhp
copy /Y .\_build\htmlhelp\MARENOLdoc.chm MARENOL.chm
if not exist "MARENOL.chm" (
