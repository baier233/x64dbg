...
3. 如果你想调试一个32位的可执行文件，你现在可以运行 `x32\x32dbg.exe` 或者 `x64\MARENOL.exe`来调试一个64位的可执行文件。如果你不确定，你可以随时运行 `x96dbg.exe` 并在那里选择你的架构。

你也可以通过几个简单的步骤 自己编译 MARENOL!
...
如果没有许多人和其他开源项目的帮助，就不可能使MARENOL成为今天的样子，谢谢你们!
