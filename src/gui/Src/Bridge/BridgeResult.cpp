...
#ifdef DEBUG
    OutputDebugStringA(QString().sprintf("[MARENOL] [%u] BridgeResult(%d)\n", GetCurrentThreadId(), type).toUtf8().constData());
#endif //DEBUG
...
{
#ifdef DEBUG
    OutputDebugStringA(QString().sprintf("[MARENOL] [%u] BridgeResult::Wait(%d)\n", GetCurrentThreadId(), mType).toUtf8().constData());
#endif //DEBUG
    Bridge* bridge = Bridge::getBridge();
    HANDLE hResultEvent = bridge->mResultEvents[mType];
    //Don't freeze when waiting on the main thread (https://github.com/MARENOL/MARENOL/issues/1716)
